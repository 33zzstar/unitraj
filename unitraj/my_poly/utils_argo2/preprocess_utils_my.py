...
with open('/zzs/everything-polynomial/Argo2/priors_argo2/vehicle/vehicle_5s.json', "r") as read_file:
    prior_vechicle = json.load(read_file)
    
with open('/zzs/everything-polynomial/Argo2/priors_argo2/cyclist/cyclist_5s.json', "r") as read_file:
    prior_cyclist = json.load(read_file)
    
with open('/zzs/everything-polynomial/Argo2/priors_argo2/pedestrian/pedestrian_5s.json', "r") as read_file:
    prior_pedestrian = json.load(read_file)
    
with open('/zzs/everything-polynomial/Argo2/priors_argo2/ego/ego_5s.json', "r") as read_file:
...
def process_argo2_data_with_scenario_parquet_my(src_file, 
                                             output_path=None, 
                                             process_track = True,
                                             process_map = True):
    ...
    ###3.智能体筛选
     # 获取所有参与者ID
    actor_ids = list(np.unique(list(scenario_data['tracks'].keys())))
    historical_df = {agent: {key: scenario_data['tracks'][agent]['state'][key][:HIST_LEN] for key in scenario_data['tracks'][agent]['state'].keys()}  
                     for agent in scenario_data['tracks'].keys()}



    # # 过滤掉观测次数少于MIN_OBS_LEN的参与者
    ...
    # # 根据筛选后的actor_ids更新数据框
    # historical_df = historical_df[historical_df['track_id'].isin(actor_ids)]
    # df = df[df['track_id'].isin(actor_ids)]
    ###4.坐标系转换

    # 获取AV和Agent的索引和数据
    av_df = scenario_data['tracks']['AV']
    av_index = list(scenario_data['tracks'].keys()).index('AV')
    focal_agt_df = scenario_data['tracks'][focal_track_id]
    focal_agt_index = list(scenario_data['tracks'].keys()).index(focal_track_id)
    #初始化各种数组用于存储处理后的数据
    num_actors = len(scenario_data['tracks'].keys()) # 智能体数量
    ...
    #计算场景中心点（以关注对象在HIST_LEN-1时刻的位置为原点）
    origin = np.array([focal_agt_df['state']['position'][:, 0][HIST_LEN-1], focal_agt_df['state']['position'][:, 1][HIST_LEN-1]])
    theta = np.array(focal_agt_df['state']['heading'][HIST_LEN-1])
    rotate_mat = np.array([[np.cos(theta), -np.sin(theta)],
                            [np.sin(theta), np.cos(theta)]])
    R_mat = np.kron(np.eye(HIST_DEGREE+1), rotate_mat)
    #处理自车轨迹
    ego_positions = np.array([av_df['state']['position'][:, 0][:HIST_LEN], av_df['state']['position'][:, 1][:HIST_LEN]]).T
    ego_headings = np.array(av_df['state']['heading'][:HIST_LEN])
    ego_velocities = np.array([av_df['state']['velocity'][:, 0][:HIST_LEN], av_df['state']['velocity'][:, 1][:HIST_LEN]]).T
    ...
    if process_track:
        ...
        for actor_id, actor_df in scenario_data['tracks'].items():
            actor_idx = actor_ids.index(actor_id)
            agent_id[actor_idx] = actor_id
            #获取历史时间步和所有时间步的索引
            actor_hist_steps = list(range(len(scenario_data['metadata']['ts'][:HIST_LEN])))
            actor_steps = timesteps
            timestep_mask[actor_idx, actor_steps] = True
            #获取物体类型和轨迹类别
            actor_type = 'UNKNOWN' if actor_df['type'] == 'OTHER' else actor_df['type']
            objects_type[actor_idx] = OBJECT_TYPES.index(actor_type)
            # objects_type[actor_idx] = OBJECT_TYPES.index(actor_df['type'])
            if actor_id == focal_track_id: # focal track
                tracks_category[actor_idx] = 3
            # elif actor_id in scored_track_id: # scored track  也需要评分的轨迹
            #     tracks_category[actor_idx] = 2
            # elif np.sum(actor_time_mask[:HIST_LEN]) < HIST_LEN: # track_fragment 不完整轨迹片段
            #     tracks_category[actor_idx] = 0
            else: # unscored track 未评分的轨迹 上下文使用
                tracks_category[actor_idx] = 1
            #获取物体轨迹数据
            positions = np.array([actor_df['state']['position'][:, 0],actor_df['state']['position'][:, 1]]).T
            headings = np.array(actor_df['state']['heading'])
            velocities = np.array([actor_df['state']['velocity'][:, 0], actor_df['state']['velocity'][:, 1]]).T
            # 组合原始轨迹数据
        ...
        poly_track_infos = {
                    ...
                    'timestep_x_mask': timestep_mask[:, :HIST_LEN], #[N, 50]
                    'timestep_y_mask': timestep_mask[:, HIST_LEN:], #[N, 60]
                    'time_window': time_window, # [N, 2]
                    'av_index': av_index,
                    'agent_index': focal_agt_index,
        ...
        scenario_data['poly_track_infos'] = poly_track_infos
        poly_track_infos.update(file_infos)
        #src_file 去掉。pkl后缀
        src_file_no_pkl = os.path.basename(src_file).replace('.pkl', '')
        poly_track_output_file = os.path.join(output_path, f'{src_file_no_pkl}_poly_infos_updated.pkl')

        with open(poly_track_output_file, 'wb') as f:
...
def process_argo2_data_with_scenario_parquet(src_file, output_path=None, process_track=True, process_map=True):
    """处理单个场景数据
    参数:
        src_file: 源数据目录
        output_path: 输出路径
...
def get_infos_from_data_my(data_path, 
                        output_path=None, 
                        num_workers=16,
                        process_track = True,
                        process_map = True):
    ...
    """处理单个pkl文件
        ...
        """
    src_files = glob.glob(os.path.join(data_path, "**/sd*.pkl"), recursive=True)
    src_files.sort() 
    results, error_files= [], []
    
# src_files=["/data1/data_zzs/AG2_offical/train/00a0a3e0-1508-45f2-9cf5-e427e1446a33/"]
    for src_file in tqdm(src_files):
        # try:
            # 获取文件所在目录
            
            result = process_argo2_data_with_scenario_parquet_my(
                src_file, 
                output_path,
...
def create_infos_from_data_my(raw_data_path,      # 原始数据存储路径
    ...
    for split in splits:
        ...
        data_infos = get_infos_from_data_my(
            data_path=os.path.join(raw_data_path),  # 原始数据路径
            output_path=os.path.join(output_path),  # 处理结果输出路径
